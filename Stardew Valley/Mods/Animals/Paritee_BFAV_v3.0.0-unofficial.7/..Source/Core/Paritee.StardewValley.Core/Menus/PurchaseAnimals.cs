...
{
    public class PurchaseAnimals : Menus.Menu
    {
        // MP: Experimental change; setting a constant to number of icons per row and changing it. default is 3
        const int iconsPerRow = 5;
        public static int GetIconsPerRow(global::StardewValley.Menus.PurchaseAnimalsMenu menu)
        {
            return iconsPerRow;
        }
        ...
        {
            return Utilities.Reflection.GetFieldValue<global::StardewValley.FarmAnimal>(menu, "animalBeingPurchased");
        }
        ...
        {
            Utilities.Reflection.GetField(menu, "animalBeingPurchased").SetValue(menu, animal);
        }
        ...
        {
            List<ClickableTextureComponent> components = Menus.PurchaseAnimals.GetAnimalsToPurchaseComponents(menu, stock, icons, out iconHeight);

            Menus.PurchaseAnimals.SetAnimalsToPurchase(menu, components);
        }

        public static int GetPriceOfAnimal(global::StardewValley.Menus.PurchaseAnimalsMenu menu)
        {
            return Utilities.Reflection.GetFieldValue<int>(menu, "priceOfAnimal");
        }

        public static void SetPriceOfAnimal(global::StardewValley.Menus.PurchaseAnimalsMenu menu, int price)
        {
            Utilities.Reflection.GetField(menu, "priceOfAnimal").SetValue(menu, price);
        }
        ...
        {
            return Utilities.Reflection.GetFieldValue<global::StardewValley.Buildings.Building>(menu, "newAnimalHome");
        }
        ...
        {
            Utilities.Reflection.GetField(menu, "newAnimalHome").SetValue(menu, home);
        }
        ...
        {
            return Menus.PurchaseAnimals.GetOkButton(menu) != null;
        }

        public static bool HasTappedOkButton(global::StardewValley.Menus.PurchaseAnimalsMenu menu, int x, int y)
        {
            return Menus.PurchaseAnimals.HasOkButton(menu) && Menus.PurchaseAnimals.TappedOnButton(Menus.PurchaseAnimals.GetOkButton(menu), x, y);
        }
        ...
        public static void SetWidth(global::StardewValley.Menus.PurchaseAnimalsMenu menu, int width)
        {
            menu.width = width;
        }
        ...
        public static List<ClickableTextureComponent> GetAnimalsToPurchaseComponents(global::StardewValley.Menus.PurchaseAnimalsMenu menu, List<global::StardewValley.Object> stock, Dictionary<string, Texture2D> icons, out int iconHeight)
        {
            ...
            for (int index = 0; index < stock.Count; ++index)
            {
                ...
                Rectangle bounds = new Rectangle(menu.xPositionOnScreen + IClickableMenu.borderWidth + index % iconsPerRow * 64 * 2, 
                                                 menu.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth / 2 + index / iconsPerRow * 85, 128, 64);
                ...
                ClickableTextureComponent textureComponent = new ClickableTextureComponent(name, bounds, label, hoverText, texture, sourceRect, scale, drawShadow)
                {
                    item = obj,
                    myID = index,
                    rightNeighborID = index % iconsPerRow == 2 ? -1 : index + 1,
                    leftNeighborID = index % iconsPerRow == 0 ? -1 : index - 1,
                    downNeighborID = index + iconsPerRow,
                    upNeighborID = index - iconsPerRow
                };
        ...
        {
            return (int)Math.Ceiling((float)Menus.PurchaseAnimals.GetAnimalsToPurchase(menu).Count / iconsPerRow); // Always at least one row
        }

        public static void AdjustHeightBasedOnIcons(global::StardewValley.Menus.PurchaseAnimalsMenu menu, int iconHeight)
        {
            Menus.PurchaseAnimals.SetHeight(menu, (int)(iconHeight * 2f) + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth / 2 + Menus.PurchaseAnimals.GetRows(menu) * 85);
        }
    }
}
